...
def get_p(mu,sigma):
    '''
    '''
    scores = np.arange(round(mu-3*sigma),round(mu+3*sigma)+1)-0.5
    probs = normpdf(scores,mu,sigma)
    mask = np.where(scores > 0)

    return np.trapz(probs[mask],dx=1)
...
def make_record_curves(records,wins,schedule):
    '''
    '''
    sigma = {team: math.sqrt(sum([x*(1-x) for x in records[team] if x != 0])) for team in records.keys()}
    mu = {team: wins[team]+sum(records[team]) for team in records.keys()}
    ...
    for round_ in brack:
        ...
        for game in round_:
            if len(game) == 1:
                ...
                win1 = {team: team1[team]*sum([team2[opp]*hth[team][opp] for opp in teams if opp != team]) for team in teams}
                win2 = {team: team2[team]*sum([team1[opp]*hth[team][opp] for opp in teams if opp != team]) for team in teams}
...
def _538_table(sch_csv, scr_csv, oth_csv):
    ...
    if weeks_left:
        ...
        playoff = find_top(cdf,record_curves,4)
        consolation = find_top(cdf,record_curves,8)
        first = playoffs(4,hth,cdf,record_curves)
    else:
        print('Work in progress')
    write_table('Odds.csv',['consolation','playoffs','champion'],consolation,playoff, first[1])
